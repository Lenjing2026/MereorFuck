 ...
 *
 * 本类只提供函数，不注册任何监听。由调度器在 {@code BlockBreakEvent} 中调用
 * {@link #onBlockBreak(Player, Block, Material)}，并每秒调用 {@link #tick()} 即可。
 *
 * 服务端能看到的信号
 *
 * 透视客户端不会向服务端暴露任何额外信息，服务端唯一能观察到的是「破坏了哪些方块」，
 * 因此检测只能建立在行为统计上：
 *
 * 1. 挖到未暴露的稀有矿 —— 被破坏的矿石六个面全是实体方块，没有任何空气 / 水可达的面。
 *    原版玩家只能挖到自己看得见的方块，这种矿石正常情况下根本挖不到，
 *    只有透视外挂才会「隔墙直挖」（最不容易误判的信号）；
 * 2. 稀有矿命中率异常 —— 按固定采样窗口统计「挖到的方块」中稀有矿（钻石 / 绿宝石 / 远古残骸）
 *    的占比。正常深板岩层挖矿约 1%～3%，透视外挂普遍在 20% 以上。
 *
 * 如何区分正常行为
 *
 * - 只统计玩家主动破坏的方块：爆炸、活塞、命令、自然破坏都不会进入统计；
 * - 只把钻石、绿宝石、远古残骸算作稀有矿；金 / 铁 / 铜 / 下界金矿等常见矿不计入，
 *   避免下界挖金、山地挖铁被误判；
 * - 命中率检查必须累计满一整个采样窗口（默认 120 个方块）且稀有矿数量达到下限才判定，
 *   偶尔撞上一条大矿脉不会触发；
 * - 创造 / 旁观模式、{@code meteorfuck.bypass}（OP 默认拥有）完全免检；
 * - 换世界 / 加入 / 重生后清空统计窗口，避免跨环境数据串味；
 *   长时间不挖矿也会重置窗口并衰减违规等级；
 * - 挖到未暴露矿石时若相邻区块尚未加载、或位于世界边界，一律保守放行；
...
public final class No_XRay {
	 ...
	 *
	 * 建议在 {@code EventPriority.MONITOR} 下调用，此时方块尚未被移除，
	 * 可以正确判断它是否「有暴露面」。
	 *
	 * @param player 玩家
	 * @param block  被破坏的方块
	 ...
	 * 是否为需要计入统计的稀有矿。
	 *
	 * 刻意排除金 / 铁 / 铜 / 下界金矿等常见矿：它们在各自地形里被大量挖掘属于正常行为。
	 */
	 ...
	 *
	 * 相邻区块未加载、或位于世界高度边界时一律返回 {@code true}（视为暴露），
	 * 宁可漏判也不误判。
	 */
