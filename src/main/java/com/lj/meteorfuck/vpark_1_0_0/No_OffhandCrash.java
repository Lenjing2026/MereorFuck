 ...
 *
 * <p>本类<b>只提供函数</b>，不注册任何事件监听。由调度器在
 * {@code PlayerSwapHandItemsEvent}、{@code InventoryClickEvent}、{@code InventoryDragEvent}
 * 中转发，并每秒调用 {@link #tick()} 即可。</p>
 *
 * <p><b>注意</b>：这是唯一会要求调度器<b>取消事件</b>的模块（各入口返回 {@code true}
 * 表示「请取消这次事件」）。原因是它防的是<b>服务器崩溃</b>，
 * 必须在事件阶段就掐断，而不是等违规等级攒够。</p>
 *
 * <h3>原理</h3>
 * <p>副手槽位属于「实体装备」，每次变化服务端都要重新走一遍交换 + 广播流程。
 * 彗星端（Meteor Client）「杂项」分类里的「副手崩溃」模块就是滥用这一点：
 * 它<b>拦截并狂发 {@code PlayerActionC2SPacket}（动作 {@code SWAP_ITEM_WITH_OFFHAND}）</b>，
 * 每个游戏刻循环发送 {@code speed} 次 —— <b>默认 2000 次、最高 10000 次</b>，
 * 也就是每秒数万到数十万个数据包，靠榨干服务端（以及收到海量广播的周边玩家）的处理能力
 * 来让别人掉线甚至崩溃。</p>
 *
 * <p>所以本模块按「数据包洪水」而不是「手感异常」来设计：只看副手切换与背包点击的
 * <b>真实时间频率</b>，越过洪水门槛就立刻取消事件并踢人 —— 取消事件会直接掐断那次交换
 * 与随之而来的广播，攻击效果当场失效，这是本模块最大的价值。</p>
 *
 * <h3>检测项</h3>
 * <ol>
 *   <li><b>副手切换洪水</b> —— 100 毫秒内切换 ≥ 64 次（洪水级，直接踢），
 *       或 1 秒内切换 ≥ 30 次并连续两个窗口都超（持续洪水）；
 *       容器里的「副手交换」点击（{@code SWAP_OFFHAND} / {@code HOTBAR_SWAP}）同样计入；</li>
 *   <li><b>背包点击洪水</b> —— 100 毫秒内点击 ≥ 64 次（直接踢），
 *       或 1 秒内点击 ≥ 40 次并连续两个窗口都超；</li>
 *   <li><b>原版不可能发出的点击包</b> —— 槽位越界、{@code ClickType/InventoryAction.UNKNOWN}
 *       （客户端发来的非法模式会被映射为 UNKNOWN）、热键栏按钮不在 0-8 之间；</li>
 *   <li><b>拖动槽位数越界</b> —— 一次拖动覆盖的槽位数量超过整个界面本身。</li>
 * </ol>
 *
 * <h3>如何区分正常行为</h3>
 * <ul>
 *   <li>洪水阈值取的是「每秒数百次」级别，远高于人类极限（按 F 最快也就 10 次 / 秒左右），
 *       正常玩家无论怎么连点、整理背包都碰不到；</li>
 *   <li>速率类判定还要求<b>连续两个窗口都超</b>：网络抖动或服务端卡顿把玩家几次正常操作
 *       合并送达时只是一次性突发，下一个窗口就会掉回去，因此不会被误判；</li>
 *   <li>两手是否拿着物品<b>不影响</b>统计：这个漏洞空手也照样刷，服务端每个包都要走一遍
 *       交换与广播流程，所以不能像其它检测那样要求「必须手持物品」；</li>
 *   <li>槽位越界判定使用「整个界面槽位数」，因此所有合法槽位（含窗口外 -1、光标 -999）都放行；</li>
 *   <li>只有 {@code meteorfuck.bypass}（OP 默认拥有）与死亡状态免检 ——
 *       本模块<b>刻意不按游戏模式放行</b>：创造模式玩家同样能把副手刷爆，
 *       而崩溃一次是全服一起承担，所以这里不做创造 / 旁观豁免；</li>
 *   <li><b>没有宽限期</b>：崩溃防护不能等玩家进服几秒后才生效；</li>
 *   <li>洪水级突发与原版不可能出现的包直接踢出，普通速率异常按加权累计（VL）处理。</li>
 * </ul>
 */
public final class No_OffhandCrash {
	 ...
	 *
	 * <p>彗星端的「副手崩溃」正是靠狂刷这个数据包实现的，因此这里是本模块的主战场：
	 ...
	 * 判断玩家是否完全免检。
	 *
	 * <p>这里刻意<b>不</b>按游戏模式放行：创造模式同样能把副手刷爆，
	 * 而崩溃是全服一起承担，所以只放行权限豁免与死亡状态。</p>
	 */
